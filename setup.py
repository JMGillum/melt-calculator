...
def SetupParser():
    ...
    collection_manage_parser.add_argument(
        ...
        "--delete",
        action="store_true",
        help="Switches to delete mode. (Deletes the entry from the purchases table)",
    )
    ...
    if not hide_price:
        ...
        for key in prices:
            ...
            if config["enforce_prices_set"] and price < 0:
                print(
                    f"WARNING: PRICE FOR [{key}]({name.title()}) HAS NOT BEEN SET. PLEASE UPDATE DATABASE BEFORE CONTINUING..."
                )
                exit(1)
        ...
        for key in prices:
            ...
            if not hide_price:
                ...
                if not printed:
                    print(
                        f"{name.title()} spot: {config['currency_symbol']}{price:.2f} as of: {date}"
                    )
