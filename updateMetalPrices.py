...
def GetMetalPricesFromUser(db:DB_Interface, metals:dict, config:dict):
    ...
    for metal_id, metal in metals.items():
        while True:
            ...
            price = input(
                f"Enter Price for [{metal_id}]({name}) (currently:{config['currency_symbol']}{price}) (enter empty string to skip): "
            )
            if price:  # The user entered a valid price
                ...
                response = input(
                    f"Enter date (YYYY-MM-DD) for price or enter empty string to use {current_date}: "
                )
                ...
                if input(
                    f"Press enter to update [{metal_id}]({name}) to {config['currency_symbol']}{price} as of {price_date} (enter any other key to modify): "
                ):
    ...
    if updates:
        ...
        if confirmation == "y" or confirmation == "yes":
            ...
            for (
                item,
                result,
            ) in results:  # Status as to whether the metal was updated successfully.
                print(
                    f"{item} was updated {'successfully' if result else 'unsuccessfully'}"
                )
