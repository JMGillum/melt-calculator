...
def addCountry(information):
    ...
    log(query_string)
    return (country_id,query_string)
    """
...
def addDenomination(information):
    ...
    return (f"{prefix}_{code}",query_string)
    """
...
def addCoin(information):
    ...
    query_string += ");"
    log(query_string)
    return_queries_coins = query_string
    ...
    log(query_string)
    #queries_coins.append(query_string)
    return (f"{prefix}_{code}",return_queries_coins,query_string)


def AddCoins(db, output_prefix, config):
    ...
    while True:
        ...
        # --- Example: Select Data ---
        print("\nSelecting data...")
        results = list(db.CheckExistence(True,{"id":information['coin_prefix'],"id_type":"value"}))
        """
        ...
        if results or [
            ...
            if matches:  # See if any added coins match
                ...
                results = list(db.CheckExistence(True,{"id":f"{information['coin_prefix']}%","id_type":"coin","use_like":True}))
                """
                ...
                if results:
                    information["coin_number"] = (
                        int(results[-1][0].split("_")[-1]) + 1
                    )  # Gets count value at end of coin_id
        else:  # No value found
            print("No data found")
            print("\nSelecting data...")
            results = list(db.CheckExistence(True,{"id":information['value_prefix'],"id_type":"denomination"}))
            """
            ...
            if not results and not [
                ...
                print("No data found")
                print("\nSelecting data...")
                results = list(db.CheckExistence(True,{"id":information['denomination_prefix'],"id_type":"country"}))
                """
                select_query = (
                ...
                if not results and not [
                    ...
                    val = addCountry(information)
                    added_countries.append(val[0])
                    queries_countries.append(val[1])

                val = addDenomination(information)
                added_denominations.append(val[0])
                queries_denominations.append(val[1])
                
            val = addValue(information)
            added_values.append(val[0])
            queries_values.append(val[1])
        val = addCoin(information)
        added_coins.append(val[0])
        queries_coins.append(val[1])
        queries_coins.append(val[2])
        queries_years.append(val[2])
        print(
    ...
    if response == "y" or response == "yes":
        ...
        if output_prefix:
            environment_prefix = f"{output_prefix}_"
